locals {
  ...
  usc_singapore_nat_cidr = cidrsubnet(var.ucs_singapore_cidr, 8, 1)
...
resource "huaweicloud_vpc_subnet" "usc_singapore_nat" {
  name = "usc_singapore_nat"
  cidr = local.usc_singapore_nat_cidr

  gateway_ip = cidrhost(local.usc_singapore_nat_cidr, 1)
  vpc_id     = huaweicloud_vpc.ucs_singapore.id
}
...
resource "huaweicloud_cce_cluster" "ucs_singapore" {
  ...
  vpc_id                 = huaweicloud_vpc.ucs_singapore.id
  subnet_id              = huaweicloud_vpc_subnet.usc_singapore_cce_master.id
  container_network_type = "eni"
  eni_subnet_id          = huaweicloud_vpc_subnet.usc_singapore_cce_pod.ipv4_subnet_id
}
...
resource "huaweicloud_nat_gateway" "ucs_singapore" {
  ...
  spec        = "1"
  vpc_id      = huaweicloud_vpc.ucs_singapore.id
  subnet_id   = huaweicloud_vpc_subnet.usc_singapore_nat.id
}
...
resource "huaweicloud_nat_snat_rule" "ucs_singapore_cce_pod" {
  ...
  subnet_id      = huaweicloud_vpc_subnet.usc_singapore_cce_pod.id
}